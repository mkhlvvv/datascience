...
#last_date<-as.POSIXlt("2021-03-04")


 #table_suffix<-"20210223"
...
#         token_path = "C:/R/mytarget")

campaing <- myTarGetCampaignList(login = "my_test_client", token_path = "C:/R/mytarget", request_speed = 1.2)
...
#yamarketrAuth(Login = "llc-ut@yandex.ru", NewUser = TRUE, TokenPath = "C:/R/token_yandex")

campaigns <- yamarketrGetCampaigns(Login = NULL, TokenPath = "C:/R/token_yandex")

yamarketCosts <- yamarketrGetCosts(campaigns,
                                   fromDate = format(last_date, "%d-%m-%Y"),
                                   toDate = format(last_date, "%d-%m-%Y"),
                                   Login = NULL,
                                   TokenPath = "C:/R/token_yandex",
                                   places = 0,
                                   model = 0,
                                   fetchBy = "daily")
...
clicks,
spend",
  date_start         = (as.POSIXlt(last_date)+86400),
  date_stop          = (as.POSIXlt(last_date)+86400))
options(rfacebookstat.access_token = "EAACg7dbgLXMBAI1k8ZB3gPEBPFEXB9f6TBhM4TQbneTHevDZCcfcK2fcGJrsRn4xLgL1zWY3ROHUcLu3KCKo51rrEExKzj40fzE91qwS5bon664GwHGUk0z4FgQJaSFPYvnH6vQFO64tje7aeIdZAT1NKZCZAAImyo55HHzUEZBiruDZCv6Jd6W",
...
users_path_sks<- get_ga("ga:186429948",
                        ...
                        metrics     = "ga:users",
                        filters     = "ga:pagePath=@obzor-tovarov.pro",
                        segment    = "users::sequence::ga:pagePath=@obzor-tovarov.pro;->>ga:pagePath=@sendo-air-90-akcija,ga:pagePath=@product/air-pro-4;->>ga:goal5Completions>0,ga:goal6Completions>0",
                        fetch.by      = "month",
                        samplingLevel =  "HIGHER_PRECISION",
                        #max.results   = 10000,
                        token = rga_auth)
...
users_path<- get_ga("ga:186429948",
                    ...
                    metrics     = "ga:users",
                    filters     = "ga:pagePath=@obzor-tovarov.pro",
                    segment    = "users::sequence::ga:pagePath=@obzor-tovarov.pro;->>ga:pagePath=@garlyn.ru,ga:pagePath=@bist.ru,ga:pagePath=@mirandi.ru;->>ga:goal8Completions>0",
                    fetch.by      = "month",
                    samplingLevel =  "HIGHER_PRECISION",
                    #max.results   = 10000,
                    token = rga_auth)
